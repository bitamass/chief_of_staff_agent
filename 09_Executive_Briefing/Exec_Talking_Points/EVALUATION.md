# Evaluation — Exec Talking Points

The result passes when:

- It answers the specific exec talking points question.
- Every material claim is traceable to dated evidence.
- The concise executive talking points has the required fields and accountable owners.
- Unknowns, conflicts, and assumptions are explicit.
- Recommendations are practical and clearly separated from facts.
- Cite sources, state limitations, and keep recommendations separate from facts.